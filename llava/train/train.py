...
def preprocess_mpt(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            conv.append_message(role, sentence["value"])
        p = conv.get_prompt()
        # print(f"Prompt: has image {has_image}. prompt: {p}")
...
def preprocess(
    ...
    if (
        ...
        or conversation_lib.default_conversation.version == "tiny_llama"
    ):
...
def train(attn_implementation=None):
    ...
    if model_args.version == "v0":
        ...
        print(
            f"Using conversation version: {conversation_lib.default_conversation.version}"
        )
